import pytest
from credentials import vemail, vpassw, vusername
...
def auth_and_goto_mypets():
    ...
    pytest.driver.find_element(By.ID, 'email').send_keys(vemail)
    pytest.driver.find_element(By.ID, 'pass').send_keys(vpassw)
...
def test_pet_quantity(auth_and_goto_mypets):
    ...
    assert auth_and_goto_mypets.find_element(By.TAG_NAME, 'h2').text == vusername
